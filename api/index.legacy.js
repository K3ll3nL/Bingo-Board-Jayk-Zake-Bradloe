...
// ─────────────────────────────────────────────────────────────────────────────
// Month Statistics + Community Tier List
// See gan-harness/spec.md for the full contract these three routes implement.
// ─────────────────────────────────────────────────────────────────────────────
...
// Aggregations are split into standard and restricted buckets (mirrors the
// gameStats/restrictedGameStats pattern from /api/mod/board-builder's
// buildGameStats) based on entries.restricted_submission, with a combined
// top-level overview for consumers that do not care about the split (e.g.
// HomeHighlights teaser).
app.get('/api/stats/month', async (req, res) => {
  try {
    ...
    const [
      ...
      { data: tierSubmissions, error: tierError },
    ] = await Promise.all([
      supabase.from('entries').select('user_id, pokemon_id, game, restricted_submission').eq('month_id', monthId),
      supabase.from('bingo_achievements').select('bingo_type').eq('month_id', monthId),
      supabase.from('tier_list_submissions').select('tiers').eq('month_id', monthId),
    ]);
    ...
    const standardEntries = allEntries.filter(e => !e.restricted_submission);
    ...
    const summarize = (subset) => {
      ...
      subset.forEach(e => { catchCountByMon[e.pokemon_id] = (catchCountByMon[e.pokemon_id] || 0) + 1; });
      ...
      const catches_by_game = Object.entries(catchCountByGame)
        .sort((a, b) => b[1] - a[1])
        .map(([game, count]) => ({ game, label: GAME_LABELS[game] || game, catch_count: count }));
    ...
    const standardSummary = summarize(standardEntries);
    const restrictedSummary = summarize(restrictedEntries);

    // Resolve display data (pokemon fields, user display names) once for the
    // union of both buckets top ids - avoids double-fetching shared mons/users.
    const allTopMonIds = [...new Set([...standardSummary.topMonIds, ...restrictedSummary.topMonIds])];
    const allTopUserIds = [...new Set([...standardSummary.topUserIds, ...restrictedSummary.topUserIds])];
    ...
    const buildTopHunted = (summary) => summary.topMonIds.map(id => ({
      pokemon_id: id,
      name: monById[id]?.name || 'Unknown',
      national_dex_id: monById[id]?.national_dex_id ?? null,
      catch_count: summary.catchCountByMon[id],
      // Extra gender/form fields for PokemonImage - additive, does not change the
      // documented spec.md response shape (consumers that only read the 4 spec
      // fields above are unaffected).
      form_id: monById[id]?.form_id ?? null,
      forms_count: monById[id]?.forms_count,
      genderless: monById[id]?.genderless,
      has_gender_difference: monById[id]?.has_gender_difference,
      has_major_gender_difference: monById[id]?.has_major_gender_difference,
      custom_gender_code: monById[id]?.custom_gender_code,
    }));

    const buildMostActive = (summary) => summary.topUserIds
      .map(id => ({ user_id: id, display_name: userById[id]?.display_name || 'Unknown', catch_count: summary.catchCountByUser[id] }))
    ...
    const standard = buildBucket(standardSummary);
    const restricted = buildBucket(restrictedSummary);

    const achievement_counts = { row: 0, column: 0, x: 0, blackout: 0 };
    (achievements || []).forEach(a => { if (achievement_counts[a.bingo_type] !== undefined) achievement_counts[a.bingo_type] += 1; });
    ...
    const submissionCount = (tierSubmissions || []).length;
      ...
      if (topEntry && topEntry[1] > 0) {
        ...
        if (mon) {
          consensus_callout = {
            ...
            pokemon_id: mon.id,
            name: mon.name,
            national_dex_id: mon.national_dex_id,
            tier: 'super_hard',
    ...
    res.json({
      ...
      // Combined (standard + restricted) totals - kept at top level for
      // consumers that do not care about the split, e.g. the HomeHighlights teaser.
      overview: {
...
// GET /api/tier-list — this month's board pool, the community consensus, and
// (if authenticated) the viewer's own draft/saved submission.
app.get('/api/tier-list', async (req, res) => {
  try {
    ...
    const [
      ...
      supabase.from('monthly_pokemon_pool').select('pokemon_id').eq('month_id', monthId),
      supabase.from('tier_list_submissions').select('user_id, tiers, submitted_at').eq('month_id', monthId),
    ]);
    if (poolError) throw poolError;
    ...
    let pokemonRows = [];
    if (poolIds.length) {
      const { data, error } = await supabase.from('pokemon_master').select(POKEMON_IMAGE_FIELDS).in('id', poolIds);
      if (error) throw error;
      pokemonRows = data || [];
    }

    // Preserve stable ordering by national dex number so the pool renders consistently.
    const pool = pokemonRows
      .slice()
    ...
    // Per-mon consensus distribution across all submissions for this month.
    const consensus = poolIds.map(pokemonId => {
    ...
    // null (not { submitted: false }) signals "logged out" — distinct from "logged in, not started".
    let viewer_submission = null;
    if (viewerId) {
      const mine = (submissions || []).find(s => s.user_id === viewerId);
      viewer_submission = {
...
// POST /api/tier-list — upsert the authenticated user's tier submission for a month.
app.post('/api/tier-list', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      submitted_at: submittedAt,
      ranked_count: Object.keys(tiers).length,
...
// ── Game Board (mod-only race board) ─────────────────────────────────────────
